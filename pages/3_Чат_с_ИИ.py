...
def response_func(prompt, text):
    ...
    reply_prompt = reply_prompt = ("Always reply in Russian language. Here is your personality:\n"
                ...
                "3) В ответах, подразумевающих прямой ответ, не перечисляй несколько разных мнений, если того не требует пользователь. Отвечай без вводных слов, если того не требует пользователь.\n"
                "4) В научных работах переходи на научный язык, старайся настроить свою речь плавно и без сложных научных высказываний, если того не требует пользователь.\n"
